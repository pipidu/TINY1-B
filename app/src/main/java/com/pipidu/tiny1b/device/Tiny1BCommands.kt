...
class Tiny1BCommands(private val connection: UsbDeviceConnection) {
    fun manualShutter() {
        runCatching { connection.controlTransfer(0x41, 0x20, 0x0000, 0x0345, null, 0, TIMEOUT_MS) }
    }

    fun setKbCalibrate(enabled: Boolean) {
        val value = if (enabled) 1 else 0
        runCatching { connection.controlTransfer(0x41, 0x20, value, 0x0341, null, 0, TIMEOUT_MS) }
    }
    ...
    fun getShutterMaxTime(): Int {
        return runCatching {
            ...
            if (n > 0) data[0].toInt() and 0xFF else -1
        }.getOrDefault(-1)
    }

    fun setShutterMaxTime(seconds: Int) {
        val data = byteArrayOf(seconds.coerceIn(1, 120).toByte())
        runCatching { connection.controlTransfer(0x41, 0x20, 0x0103, 0x03C4, data, 1, TIMEOUT_MS) }
    }
