...
- 实时热成像预览（USB UVC，Kotlin/Java，无厂商 `.so`）
- 软件 ISR 超分辨率
...
本仓库是独立产品，**不包含**厂商 demo 工程、demo APK、示例资源，也不包含 `libUVCCamera` / `libuvc` / `libusb`。厂商 zip 仅可私下对照。

## 构建

需要 JDK 21+ 与 Android SDK（compileSdk 35，targetSdk 35）。等时传输会编译一小段 USBFS ioctl（NDK）。
